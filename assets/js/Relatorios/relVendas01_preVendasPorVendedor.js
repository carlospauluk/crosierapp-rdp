...
$(document).ready(function () {
    ...
    let $filterVendedor = $('#filter_vendedor');


    $filterVendedor.select2({
        placeholder: '...',
        allowClear: true,
        data: $filterVendedor.data('options')
    });
    if ($filterVendedor.data('val')) {
        $filterVendedor.val($filterVendedor.data('val')).trigger('change');
    }

    $filterVendedor.on('select2:select', function () {
        $formPesquisar.submit();
    });
